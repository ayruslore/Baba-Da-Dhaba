...
$.ajax({
  ...
  success: function(data){
    ...
    for(var i=0; i<x.length; i++)
      DATA["Courses"][x[i]["course"]][toTitleCase(x[i]["name"].replace(/_/g, ' '))]=[x[i]["price"], x[i]["v_n"], x[i]["Old_Aiport_Road"]];
    console.log(DATA);
...
$.ajax({
  type: "GET",
  url: redisDb+"/hotel_status/Old_Aiport_Road",
  success: function(data){
    console.log(data);
    shutdown = data;
...
function shutingdown(){
  $.ajax({
    url: redisDb+'/shutdown_hotels/Old_Aiport_Road',
    success: function(data){
      console.log(data);
      var x = JSON.parse(data);
      console.log(x);
      console.log(x['status']);
      if(x['status']=='Hotel is shutdown'){
        alert(x['status']);
        shutdown = 1;
      }
      else{
...
function changeAvail(c, d){
  ...
  $.ajax({
    type: "GET",
    url: redisDb+"/outofstock_O/"+d.replace(/ /g, '_').toLowerCase(),
    success: function(data){
      viewMenu();
    },
    error: function(data){
